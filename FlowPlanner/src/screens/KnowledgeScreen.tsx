...
function KnowledgeEditor({ visible, initial, onClose, onSaved }: EditorProps) {
  ...
  const pickImage = async (fromCamera: boolean) => {
    ...
    try {
      // Extra Android legacy permission request (Android < 13)
      if (Platform.OS === 'android' && !fromCamera) {
        try {
          await PermissionsAndroid.request(
            PermissionsAndroid.PERMISSIONS.READ_EXTERNAL_STORAGE,
            {
              title: 'Доступ к фото',
              message: 'FlowPlanner хочет получить доступ к вашим фотографиям',
              buttonPositive: 'Разрешить',
              buttonNegative: 'Отмена',
            }
          );
          // On Android 13+ this permission is deprecated — we still proceed
        } catch (_) {}
      }
      ...
      if (perm !== 'granted') {
        Alert.alert(
          '⚠️ Нет доступа к фото',
          'Разреши доступ к галерее в Настройках телефона → Приложения → FlowPlanner → Разрешения',
          [{ text: 'OK' }]
        );
        return;
      }
      ...
      try {
        ...
        Alert.alert('Ошибка загрузки', uploadErr?.message || 'Не удалось загрузить изображение');
      }
    } catch (err: any) {
          ...
          <ScrollView contentContainerStyle={styles.editorScroll} keyboardShouldPersistTaps="handled">
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.imagesRow}>
              ...
              <TouchableOpacity
                style={[
                  styles.addImageBtn,
                  images.some((img) => img.uploading) && { borderColor: 'rgba(244,167,195,0.4)' },
                ]}
                onPress={showImagePicker}
                disabled={images.some((img) => img.uploading)}
              >
                {images.some((img) => img.uploading) ? (
                  <ActivityIndicator size="small" color="#f4a7c3" />
                ) : (
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        contentContainerStyle={[notes.length === 0 ? styles.emptyContainer : styles.list, getScreenPadding(insets.bottom)]}
        ListEmptyComponent={
