...
html,body { width:100%; height:100%; background:#0d0820; overflow:hidden; font-family:'DM Sans',sans-serif; color:#f0e8ff; }
...
#sidebar {
  ...
  border-right: 1px solid rgba(244,167,195,0.12);
  display: flex;
...
.side-btn:hover { background: rgba(244,167,195,0.1); color: #f4a7c3; transform: scale(1.05); }
.side-btn.active { background: rgba(244,167,195,0.15); color: #f4a7c3; box-shadow: 0 0 12px rgba(244,167,195,0.2); }
.side-btn span { font-size: 9px; color: inherit; }

.side-sep { width: 36px; height: 1px; background: rgba(244,167,195,0.12); margin: 4px 0; }
...
.tb-ghost { background: transparent; color: #9b7fa6; border: 1px solid rgba(244,167,195,0.15); }
.tb-ghost:hover { background: rgba(244,167,195,0.08); color: #f4a7c3; }
.tb-active { background: rgba(244,167,195,0.15); color: #f4a7c3; border: 1px solid rgba(244,167,195,0.3); }
.tb-danger { background: transparent; color: #ff8fa3; border: 1px solid rgba(255,100,120,0.2); }
.tb-danger:hover { background: rgba(255,100,120,0.1); }
.tb-gold { background: #c9a84c; color: #1a0f2e; font-weight:700; }

.layer-label {
  font-size: 10px; color: #9b7fa6; margin-left: 4px;
  border-left: 1px solid rgba(244,167,195,0.15);
  padding-left: 8px;
}
...
#canvas-wrapper {
  ...
  background: radial-gradient(ellipse at center, #1a0f2e 0%, #0d0820 100%);
  position: relative; overflow: hidden;
}

/* Stars bg */
.star { position:absolute; border-radius:50%; background:#f0e8ff; pointer-events:none; }
...
.panel {
  ...
  border-right: 1px solid rgba(244,167,195,0.15);
...
.panel-title { font-size: 13px; font-weight: 700; color: #f4a7c3; }
.panel-close { background: none; border: none; color: #9b7fa6; cursor: pointer; font-size: 16px; }
.panel-close:hover { color: #f4a7c3; }
.panel-body { padding: 12px; flex:1; }

/* FRAMES GRID */
.frames-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }
.frame-item {
  aspect-ratio: 1; border-radius: 10px; cursor: pointer;
  border: 2px solid rgba(244,167,195,0.15);
  display: flex; align-items: center; justify-content: center;
  font-size: 28px; transition: all 0.2s;
  background: rgba(45,27,78,0.4);
}
.frame-item:hover { border-color: #f4a7c3; transform: scale(1.05); background: rgba(244,167,195,0.08); }

/* STICKERS GRID */
.sticker-grid { display: grid; grid-template-columns: repeat(5,1fr); gap: 6px; }
.sticker-item {
  font-size: 24px; text-align: center; padding: 6px;
  border-radius: 8px; cursor: pointer; transition: all 0.15s;
}
.sticker-item:hover { background: rgba(244,167,195,0.12); transform: scale(1.2); }

/* BG PANEL */
.bg-grid { display: grid; grid-template-columns: repeat(3,1fr); gap: 8px; }
.bg-item {
  aspect-ratio: 1; border-radius: 10px; cursor: pointer;
  border: 2px solid rgba(244,167,195,0.15); transition: all 0.2s;
  overflow: hidden;
}
.bg-item:hover { border-color: #f4a7c3; transform: scale(1.05); }
.bg-item.active { border-color: #f4a7c3; box-shadow: 0 0 10px rgba(244,167,195,0.3); }

/* SECTION LABEL */
.sec-label { font-size: 10px; color: #9b7fa6; text-transform: uppercase; letter-spacing: 1px; margin: 10px 0 6px; }

/* FONT CHIPS */
.font-chips { display: flex; flex-direction: column; gap: 6px; }
.font-chip {
  padding: 8px 12px; border-radius: 10px; border: 1px solid rgba(244,167,195,0.15);
  background: transparent; color: #f0e8ff; cursor: pointer; text-align: left;
  transition: all 0.15s; font-size: 13px;
}
.font-chip:hover, .font-chip.active { background: rgba(244,167,195,0.12); border-color: #f4a7c3; color: #f4a7c3; }
...
/* SLIDER */
.slider-row { display: flex; flex-direction: column; gap: 4px; margin: 8px 0; }
.slider-label { font-size: 11px; color: #9b7fa6; display: flex; justify-content: space-between; }
input[type=range] {
  width: 100%; height: 4px; border-radius: 2px;
  background: rgba(244,167,195,0.2); outline: none;
  -webkit-appearance: none;
}
input[type=range]::-webkit-slider-thumb {
  -webkit-appearance: none; width: 14px; height: 14px;
  border-radius: 50%; background: #f4a7c3; cursor: pointer;
}

/* TEXT MODAL */
#text-modal {
  display: none; position: fixed; inset: 0;
  background: rgba(13,8,32,0.88); z-index: 200;
  align-items: center; justify-content: center;
  backdrop-filter: blur(12px);
}
#text-modal.show { display: flex; }
.modal-box {
  background: #1e1040; border-radius: 20px; padding: 24px;
  width: 90%; max-width: 380px;
  border: 1px solid rgba(244,167,195,0.25);
  box-shadow: 0 20px 60px rgba(0,0,0,0.6);
}
.modal-box h3 { color: #f4a7c3; margin-bottom: 14px; font-size: 15px; }
.modal-box textarea {
  width: 100%; background: rgba(13,8,32,0.5);
  border: 1px solid rgba(244,167,195,0.25); border-radius: 12px;
  padding: 12px; color: #f0e8ff; font-size: 15px; resize: none;
...
.btn-cancel { background: rgba(244,167,195,0.1); color: #9b7fa6; }
.btn-confirm { background: #f4a7c3; color: #1a0f2e; }

/* CONTEXT MENU */
#ctx-menu {
  display: none; position: absolute; z-index: 100;
  background: rgba(26,15,46,0.98); border: 1px solid rgba(244,167,195,0.2);
...
.ctx-item {
  ...
  font-size: 12px; color: #f0e8ff; display: flex; align-items: center; gap: 8px;
  transition: background 0.1s;
}
.ctx-item:hover { background: rgba(244,167,195,0.1); color: #f4a7c3; }
.ctx-sep { height: 1px; background: rgba(244,167,195,0.1); margin: 4px 0; }
...
<!-- SIDEBAR -->
<div id="sidebar">
  <button class="side-btn" onclick="openPanel('bg')" id="sb-bg" title="Фон">🎨<span>Фон</span></button>
  <button class="side-btn" onclick="addImageFromFile()" title="Фото">🖼️<span>Фото</span></button>
  <button class="side-btn" onclick="showTextModal()" title="Текст">✍️<span>Текст</span></button>
  <button class="side-btn" onclick="openPanel('stickers')" id="sb-stickers" title="Стикеры">✨<span>Стикер</span></button>
  <div class="side-sep"></div>
  <button class="side-btn" onclick="openPanel('frames')" id="sb-frames" title="Рамки">🖼<span>Рамки</span></button>
  <button class="side-btn" onclick="addShape('rect')" title="Прямоугольник">⬜<span>Прямо</span></button>
  <button class="side-btn" onclick="addShape('circle')" title="Круг">⭕<span>Круг</span></button>
  <button class="side-btn" onclick="addShape('line')" title="Линия">╱<span>Линия</span></button>
  <div class="side-sep"></div>
  <button class="side-btn" onclick="openPanel('draw')" id="sb-draw" title="Рисовать">🖌️<span>Кисть</span></button>
  <div class="side-sep"></div>
  <button class="side-btn" onclick="bringForward()" title="Вперёд">⬆<span>Вверх</span></button>
  <button class="side-btn" onclick="sendBackward()" title="Назад">⬇<span>Вниз</span></button>
  <button class="side-btn" onclick="bringToFront()" title="На перёд">⏫<span>Топ</span></button>
  <button class="side-btn" onclick="sendToBack()" title="На зад">⏬<span>Низ</span></button>
  <div class="side-sep"></div>
  <button class="side-btn" onclick="openPanel('props')" id="sb-props" title="Свойства">⚙️<span>Свойства</span></button>
</div>
...
<div id="main">
  ...
  <!-- TOP BAR -->
  <div id="topbar">
    <span class="layer-label">Слои ↕</span>
    <button class="top-btn tb-ghost" onclick="groupSelected()">🔗 Группа</button>
    <button class="top-btn tb-ghost" onclick="ungroupSelected()">🔓 Разгруп</button>
    <button class="top-btn tb-ghost" onclick="duplicateSelected()">📋 Дублировать</button>
    <button class="top-btn tb-ghost" onclick="flipH()">↔ Зеркало</button>
    <button class="top-btn tb-ghost" onclick="flipV()">↕ Перевернуть</button>
    <div style="flex:1"></div>
    <button class="top-btn tb-danger" onclick="deleteSelected()">🗑 Удалить</button>
    <button class="top-btn tb-danger" onclick="clearCanvas()">🔄 Очистить</button>
    <button class="top-btn tb-gold" onclick="exportPage()">📤 Экспорт PNG</button>
  </div>
...
window.onload = () => {
  ...
  const size = Math.min(w.clientWidth - 240, w.clientHeight - 20);
...
function initStickers() {
  const emojis1 = ['🌸','🌺','🌻','🌹','🌷','🌼','🌿','🍀','🌱','🍃','🌙','⭐','✨','💫','🌟','🔮','💜','🪄','🕯️','🌌','🦋','🌈','☁️','❄️','🍂','🍁','🌊','🔥','💧','🌬'];
  const emojis2 = ['💌','📌','🗝️','📖','✒️','📝','🎀','🎁','🎶','🎵','🍓','🍰','🧁','☕','🫖','🕰️','🪞','🪴','🧸','🪆','🫶','💕','🩷','🤍','💛','🩵','🩶','🤎','🖤','💚'];
  const g1 = document.getElementById('s1');
  if (g1) {
    g1.innerHTML = '';
    [...emojis1, ...emojis2].forEach(e => {
      const d = document.createElement('div');
      d.className = 'sticker-item'; d.textContent = e;
      d.onclick = () => addSticker(e);
      g1.appendChild(d);
    });
  }
}
